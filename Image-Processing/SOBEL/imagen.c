...
/* Mandamos un apuntador a infoHeader, para que ahí se recolecte la información
de los encabezados de la imagen BMP */
unsigned char *abrirBMP(char *filename, bmpInfoHeader *bInfoHeader)
{

  	FILE *f;
  	bmpFileHeader header;     /* cabecera del archivo BMP */
	...
	if( f  == NULL )
	{
  	...
  	/* Leemos los primeros dos bytes */
  	fread( &type, sizeof(uint16_t), 1, f );
  	if( type != 0x4D42 )        /* Comprobamos que el formato sea BMP */
    	{
  	...
  	/* Leemos la información de la cabecera del archivo completa */
	...
	if( imgdata == NULL )
	{
  	...
  	/* Nos situamos en el sitio donde empiezan los datos de la imagen,
   	nos lo indica el offset de la cabecera de fichero*/
  	fseek(f, header.offset, SEEK_SET);

  	/* Leemos los datos de imagen, tantos bytes como imgsize (cuerpo de la imagen) */
...
void guardarBMP( char *filename, bmpInfoHeader *info, unsigned char *imgdata )
{
    ...
    /* Tamaño de la imagen que estamos guardando */
...
/* Imprime los datos de los encabezados que contenga la imagen BMP*/
