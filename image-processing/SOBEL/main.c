...
int main(){
    ...
    p.imagenSegmentada = reservarMemoria( info.width, info.height ); 
    ...
    memset( p.imagenSegmentada, 255, info.width * info.height);
    ...
    for (nh = 0; nh < NUM_HILOS; nh++){  
        nhs[nh] = nh;
        pthread_create( &tids[nh], NULL, gradienteSobel, (void *)&nhs[nh]);
    }
    ...
    GrayToRGB( imagenRGB, p.imagenSegmentada, info.width, info.height);
    /* Escribimos el archivo de la imagen resultante */
    guardarBMP("calle1Seg.bmp", &info, imagenRGB);
    /* Liberamos la memoria del apuntador de cada imagen */
    free( imagenRGB );
    free( p.imagenGray );
    free( p.imagenSegmentada );
